...
# Initially, start with the same as the best-performing model above: reduced_signifUni_glm
...
The initial model with all variables known to be associated with mortality (full_glm, AIC 1332) was refined to contain variables found to have significant association with mortality in univariate analysis (signifUni_glm, AIC 1321). The statistically significant variables from this mutrivariable model were further distilled using stepwise regression to include 14 covariates (reduced_signifUni_glm, AIC 1311). Based on clinical basis and ANOVA analysis of various models, interaction terms age:albumin (p=0.054) and age:icutype (p=0.016) were also included to produce the final model. Appropriate goodness of fit was demonstrated using bins as well as the Hosmer Lemeshow test, where a non-significant p value (p=0.37) was achieved and inclusion of interaction terms demonstrated an improved Brier score (0.1013 compared to 0.1022). The AUC of the ROC was 0.75 suggesting an acceptable clinical prediction model. 

Our final model suggests that:
•	Age, Type of ICU admission, minimum albumin/ Sodium/ pH/ Temperature/ Urine Output/ WBC, maximum Bilirubin/ BUN/ GCS/ Heart Rate/ Lactate/ WBC
•	as well as Age when associated with ICU type and minimum albumin
Predicts ICU mortality in adult ICU patients with ICU length of stay >48 hours.

Our goal in creating this model was a predictive model rather than an explanatory model, hence why some important values for understanding all risk factors not included
...
***
Notably, when the final model was tested on the unimputed data, it was observed that many of the variables that were noted to be significant in the imputed data, were no longer significant. A potential explanation for this may be reduced power of the unimputed dataset to produce a statistically significant result due to the magnitude of missing variables.
***
...
AIC.mv_reduced4.split.noutliers #9993 -- improvement
...
```
* The estimated coefficients, related hazard rates (exponential of these coefficients) and teh confidence interval of the hazard rates allow us to make some conclusions about clinical measures and their impact on ICU survival.
    ...
    * `BUN_max` - for every additional unit of urea, difference in mortality risk is significant, increasing by 1.1% (95% confidence between 0.8% and 1.4%).
    * `Na_max` - for every additional unit of Na, difference in mortality risk is significant, decreasing by 4% (95% confidence between 3.5% and 5.5%).
    * `pH_min` - for every additional unit of pH, risk of mortality decreases in 35% but the 95% confidence interval is large, ranging between a reduction of 4% and 55%). This could be due to the sensitivity of the measure as humans are highly unlikely to increase or decrease pH by 1 whole unit.
    * `Temp_max` - for every additional unit of temperature, mortality risk decreases by 15% (95% confidence interval between 5% and 23%).
    * `Urine_max` - for every additional 10ml or urine output, the risk of mortality decreases by 2% (confidence interval between 0% and 4%).
...
* The dataset for fitting reduces from 2,061 individuals in df1 to 299 individuals in df0. With significantly fewer data points to fit the model, we observe the following differences:
    ...
    * The confidence intervals are much wider for the model on df0 reflecting the fewer data points, which increases the uncertainty in the estimation of coefficients. 
...
The initial full model with all variables known to be associated with survival (Model 1, AIC 10136) was distilled using stepwise regression to include 18 covariates (Model 3, AIC 10113). Only significant variables (p<0.01) from this reduced model were included in the subsequent multivariable Cox proportional model (Model 4, AIC 10113). This model was subsequently adjusted to satisfy the proportional hazards assumption by including covariate - time interaction terms (dataset split at 90 days) and further adjusted to satisfy the linearity assumption by removing extreme outliers (Final Model, global test p value=0.08 AIC 10057).

Our final model suggests that:
•	Age, maximum BUN/ Na/ Temperature and minimum pH/Urine output,
•	as well as ICUtype, minimum Albumin/ HCO3/ HR and maximum Bilirubin/ GCS/ Lactate when stratified by length of stay greater or lesser than 90 days
Predicts survival in adult ICU patients with ICU length of stay >48 hours.

Predictors that had a large magnitude of effect on survival include the individual variables of age and minimum pH, as well as the stratified variables of minimum albumin, type of ICU admission and GCS.
