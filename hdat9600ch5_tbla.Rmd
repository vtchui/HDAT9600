...
image(rotate(matrix(c(nodal$r,nodal$aged),nrow=53,byrow=FALSE)),
      xlab="LEFT = r - nodal involvement, RIGHT = age over 60 or over",
...
Why is variable acid "level of acid phosphatase in the blood serum" a binary variable??? -- I saw this dataset used to have acid and age as their numeric values.. someone must have dichotomised them both at some stage (must use a cut off for acid phosphatase level?)
...
The two significant values are aged and grade<br>
When removed, the three predictors all become statistically significant P<0.05<br>

The analysis of deviance by ANOVA shows that removal of the two predictors (difference between the two models) is statistically significant, therefore we can conclude that aged and grade are not significant predictors of nodal r.
...
```
The odds show an increase of 676% of nodal involvement with a serious X-ray result compared to a non-serious X-ray result.
The 95% confidence intervals for the odds ratio would be 57%-3549%

Alternate explanation:
The odds of nodal involvement increases by 5.8x in those with a serious X-ray result (compared to a non-serious X-ray result), according to this model. The 95% confidence interval is from a 57% (or 0.6x) increase in odds to 34.5x increase in odds with a serious compared to non-serious X-ray result.
